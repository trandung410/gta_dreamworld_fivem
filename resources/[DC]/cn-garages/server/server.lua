...
AddEventHandler('cn-garages:server:setFirstData', function()
    exports['ghmattimysql']:execute("SELECT * FROM `bbvehicles`", function(vehicles)
        if vehicles ~= nil then
            ...
            for _, vehicle in pairs(vehicles) do
                ...
                elseif vehicle.state == 'unknown' then
                    exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `state` = 'impound', `parking` = '' WHERE `plate` = '" .. vehicle.plate .. "' AND `identifier` = '" .. vehicle.identifier .. "' LIMIT 1")
                end

                if vehicle.fakeplate ~= nil and vehicle.fakeplate ~= '' then
                    exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `fakeplate` = '' WHERE `plate` = '" .. vehicle.plate .. "' AND `identifier` = '" .. vehicle.identifier .. "' LIMIT 1")
                end
            end
...
AddEventHandler('cn-garages:server:impoundVehicle', function(plate)
    ...
    exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `state` = 'impound', `parking` = '' WHERE `plate` = '" .. plate .. "' AND `identifier` = '" .. xPlayer.identifier .. "' LIMIT 1")
end)
...
AddEventHandler('cn-garages:server:parkVehicle', function(garage, typ, slots, plate, stats)
    ...
    if typ == 'garages' then
        ...
        exports['ghmattimysql']:execute("SELECT `props` FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.identifier .. "' AND `plate` = '" .. plate .. "' LIMIT 1", function(props)
            if props[1] ~= nil then
                ...
                exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `stats` = '" .. json.encode(stats) .. "', `state` = 'garage', `parking` = '" .. json.encode(jsonz) .. "' WHERE `identifier` = '" .. xPlayer.identifier .. "' AND `plate` = '" .. plate .. "'")
    ...
    elseif typ == 'houses' then
        ...
        exports['ghmattimysql']:execute("SELECT `props` FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.identifier .. "' AND `plate` = '" .. plate .. "' LIMIT 1", function(props)
            if props[1] ~= nil then
                ...
                exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `stats` = '" .. json.encode(stats) .. "', `state` = 'house', `parking` = '" .. json.encode(jsonz) .. "' WHERE `identifier` = '" .. xPlayer.identifier .. "' AND `plate` = '" .. plate .. "'")
...
AddEventHandler('cn-garages:server:setVehicleOwned', function(props, stats, model)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    exports['ghmattimysql']:execute("INSERT INTO `bbvehicles` (`identifier`, `plate`, `model`, `props`, `stats`, `state`) VALUES ('" .. xPlayer.identifier .. "', '" .. props.plate .. "', '" .. model .. "', '" .. json.encode(props) .. "', '" .. json.encode(stats) .. "', 'unknown')")
end)
...
AddEventHandler('cn-garages:server:updateProps', function(props)
    ...
    local xPlayer = ESX.GetPlayerFromId(src)
    exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `stats` = '" .. json.encode(stats) .. "', `state` = 'garage', `parking` = '" .. json.encode(jsonz) .. "' WHERE `identifier` = '" .. xPlayer.identifier .. "' AND `plate` = '" .. plate .. "'")
    exports['ghmattimysql']:execute("UPDATE `bbvehicles` (`props`) VALUES ('" .. xPlayer.identifier .. "', '" .. props.plate .. "', '" .. model .. "', '" .. json.encode(props) .. "', '" .. json.encode(stats) .. "', 'unknown')")
end)
...
AddEventHandler('cn-garages:server:vehiclePayout', function(garage, plate, price, typ)
    ...
    if typ ~= 'houses' then
		if xPlayer.getMoney() >= price then
            exports['ghmattimysql']:execute("SELECT * FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.identifier .. "' AND `plate` = '" .. plate .. "' LIMIT 1", function(vehicle)
                if vehicle[1] ~= nil then
                    ...
                    if modelExists == true then
						if xPlayer.getMoney() >= price then
                            ...
                            exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `state` = 'unknown', `parking` = '' WHERE `id` = '" .. veh.id .. "'")
                            if typ == 'garages' then
...
ESX.RegisterServerCallback('cn-garages:server:getOwnedVehicles', function(source, cb, nearbyVehicles, freeSlots, name, keys)
    ...
    exports['ghmattimysql']:execute("SELECT * FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.identifier .. "'", function(vehicles)
        ...
        if vehicles ~= nil and isAuthorized == true then
            for _, vehicle in pairs(vehicles) do
                ...
                elseif status == 'impound' then
                    local parking = json.decode(vehicle.parking)
                    table.insert(vehiclesTable, {vehicle.model, vehicle.plate, stats, garageButton('Impoound', vehicle.plate, 'impound'), 'border-left-warning'})
...
ESX.RegisterServerCallback('cn-garages:server:getImpoundedVehicles', function(source, cb, name)
    ...
    exports['ghmattimysql']:execute("SELECT * FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.identifier .. "' AND `state` = 'impound'", function(vehicles)
        function payButton(plate)
...
ESX.RegisterServerCallback('cn-garages:server:isVehicleOwned', function(source, cb, plate)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    exports['ghmattimysql']:execute("SELECT * FROM `bbvehicles` WHERE `identifier` = '" .. xPlayer.identifier .. "' AND `plate` = '" .. plate .. "' LIMIT 1", function(result)
        if result ~= nil then
            cb(true)
        else
            cb(false)
...
ESX.RegisterServerCallback('cn-garages:server:hasFines', function(source, cb)
    local src = source
    local xPlayer = ESX.GetPlayerFromId(src)
    exports['ghmattimysql']:execute("SELECT * FROM `billing` WHERE `identifier` = '" .. xPlayer.identifier .. "'", function(result)
...
AddEventHandler('cn-garages:server:addHouseGarage', function(house, coord, max)
    ...
    if houseGarages[house] == nil then
        ...
        exports['ghmattimysql']:execute("SELECT * FROM `player_houses` WHERE `house` = '" .. house .. "' LIMIT 1", function(housing)
...
AddEventHandler('cn-garages:server:buyHouseGarage', function(house, identifier, source)
    local src = source
    if serverConfig['houses'][house] ~= nil then
        table.insert(serverConfig['houses'][house]['access'], identifier)
...
AddEventHandler('cn-garages:server:isPlayerVehicle', function(typ, plate, vehicle)
    if typ == 'STEAL' then
        exports['ghmattimysql']:execute("SELECT `model` FROM `bbvehicles` WHERE `plate` = '" .. plate .. "' LIMIT 1", function(result)
            if result[1] ~= nil then
                exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `fakeplate` = '%' WHERE `plate` = '" .. plate .. "' AND `model` = '" .. result[1].model .. "' LIMIT 1")
    ...
    elseif typ == 'SET' then
        if networkVehicles[vehicle] ~= nil then
            if networkVehicles[vehicle][1] == plate then
                exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `fakeplate` = '' WHERE `plate` = '" .. plate .. "' LIMIT 1")
                networkVehicles[vehicle] = nil
            else
                exports['ghmattimysql']:execute("UPDATE `bbvehicles` SET `fakeplate` = '" .. plate .. "' WHERE `plate` = '" .. networkVehicles[vehicle][1] .. "' LIMIT 1")
