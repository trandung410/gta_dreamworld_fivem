...
local blip2 = AddBlipForCoord(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z)
			...
			BeginTextCommandSetBlipName("CUSTOM_STRING")
			AddTextComponentString("Nơi bán rùa biển")
			EndTextCommandSetBlipName(blip2)

local Redzone = AddBlipForRadius(Config.SellTurtle.x, Config.SellTurtle.y, Config.SellTurtle.z , 80.0)-- you can use a higher number for a bigger zone 
...
local blip3 = AddBlipForCoord(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z)
			...
			BeginTextCommandSetBlipName("CUSTOM_STRING")
			AddTextComponentString("Nơi bán cá mập")
			EndTextCommandSetBlipName(blip3)

local Redzone1 = AddBlipForRadius(Config.SellShark.x, Config.SellShark.y, Config.SellShark.z , 80.0)-- you can use a higher number for a bigger zone 
			SetBlipHighDetail(Redzone1, true) 
			SetBlipColour(Redzone1, 1) 
			SetBlipAlpha (Redzone1, 128)			
...
for _, info in pairs(Config.MarkerZones) do
		...
		BeginTextCommandSetBlipName("CUSTOM_STRING")
		AddTextComponentString("Thuê tàu")
...
function DisplayHelpText(str)
	SetTextComponentFormat("CUSTOM_STRING")
	...
	while true do
		...
		if fishing then
			...
			if IsControlJustReleased(0, Keys['X']) then
				fishing = false
				ESX.ShowNotification("~r~Ngừng câu cá")
			end
			if fishing then
			...
			if pause and input ~= 0 then
				...
				if input == correct then
					...
					ESX.ShowNotification("~r~Cá thoát rồi")
				end
			end
		end
	...
	while true do
		...
		Wait(wait)
			if fishing then
				...
				ESX.ShowNotification("~g~Cá đang ăn mồi \n ~h~Nhấn " .. correct .. " Để bắt cá")
				input = 0
				pausetimer = 0
			end
...
AddEventHandler('fishing:break', function()
	fishing = false
	print(GetCurrentResourceName());ClearPedTasks(GetPlayerPed(-1))
end)
...
AddEventHandler('fishing:fishstart', function()
	...
	if IsPedInAnyVehicle(playerPed) then
		ESX.ShowNotification("~y~Bạn không thể câu cá bằng oto")
	else
		if pos.y >= 7700 or pos.y <= -4000 or pos.x <= -3700 or pos.x >= 4300 then
			ESX.ShowNotification("~g~Bắt đầu câu cá")
			TaskStartScenarioInPlace(GetPlayerPed(-1), "WORLD_HUMAN_STAND_FISHING", 0, true)
			fishing = true
		else
			ESX.ShowNotification("~y~Ông định câu san hô à, đi xa ra ngoài kia mới có cá")
		end
	end
    ...
    while true do
        ...
        for k in pairs(Config.MarkerZones) do
            ...
            if distance <= 1.40 then
					...
					DisplayHelpText('Bấm E để thuê tầu')
...
function OpenBoatsMenu(x, y , z)
	...
	if data.current.value == 'boat' then
		...
		TriggerEvent("chatMessage", 'Bạn đã thuê tầu', {255,0,255}, '$' .. 2500)
	...
	if data.current.value == 'boat2' then
		...
		TriggerEvent("chatMessage", 'Bạn đã thuê tầu', {255,0,255}, '$' .. 5500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat3' then
		...
		TriggerEvent("chatMessage", 'Bạn đã thuê tầu', {255,0,255}, '$' .. 6000)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat4' then
		...
		TriggerEvent("chatMessage", 'Bạn đã thuê tầu', {255,0,255}, '$' .. 7500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat5' then
		...
		TriggerEvent("chatMessage", 'Bạn đã thuê tầu', {255,0,255}, '$' .. 4500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'boat6' then
		...
		TriggerEvent("chatMessage", 'Bạn đã thuê tầu', {255,0,255}, '$' .. 3500)
		SetPedCoordsKeepVehicle(ped, x, y , z)
	...
	if data.current.value == 'police' then
		...
		TriggerEvent("chatMessage", 'Bạn đã thuê 1 chiếc thuyền')
		SetPedCoordsKeepVehicle(ped, x, y , z)
