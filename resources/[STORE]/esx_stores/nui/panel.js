window.addEventListener('message', function (event) {
	...
	if (item.showmenu){
		...
		if(item.isMarket == true){
			...
			$('#nav-bar').append(`
				<li id="sidebar-1" onclick="openPage(0)">
					...
					<span class="tooltip">Cửa hàng của bạn</span>
				</li>
				<li onclick="openPage(1)">
					<i class="fas fa-pallet"></i>
					<span class="tooltip">Mua hàng</span>
				</li>
				<li onclick="openPage(2)">
					<i class="fas fa-users"></i>
					<span class="tooltip">Thuê người giao hàng</span>
				</li>
				<li onclick="openPage(3)">
					<i class="fas fa-chart-line"></i>
					<span class="tooltip">Nâng cấp</span>
				</li>
		...
		$('#profile-stock-2').append('Dung lượng hàng tồn kho (Max: ' + stock_capacity + ')');
					...
					<div class="d-flex flex-row align-items-center"><i class="fa fa-box checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 small">
								...
								<div class="ml-3"><i class="fas fa-boxes"></i><span class="ml-2">Số lượng: ` + amount + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						<button onclick="startJob('` + key + `')" class="btn btn-blue btn-darken-2 white">Bắt đầu công việc</button>
					</div>
				</li>
			`);
							...
							<div class="card-body cleartfix">
								<div class="media align-items-stretch">
									...
									<div class="media-body">
										<h4>` + item.name + `</h4>
										<span> Giá: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(item.price_to_customer)  + `</span>
									</div>
									<div class="align-self-center">
							...
							<div class="card-body cleartfix">
								<div class="media align-items-stretch">
									...
									<div class="media-body">
										...
										<button onclick="buyItem('` + fkey + `')" class="btn btn-blue btn-darken-2 white deposit-money-btn col-6">Mua hàng</button>
									</div>
									<div class="align-self-center">
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><i class="fa fa-box checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 small">
								<div><i class="fas fa-coins"></i><span class="ml-2">Thưởng: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(store_job.reward) + `</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i><span class="ml-2">Tổng tiền: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(total_cost) + `</span></div>
								...
								<div class="ml-3"><i class="fas fa-boxes"></i><span class="ml-2">Số lượng: ` + store_job.amount + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						<button onclick="deleteJob(` + store_job.id + `)" class="btn btn-red btn-accent-4 white">Huỷ hợp đồng</button>
					</div>
				</li>
			`);
		}
			...
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card rounded shadow-sm border-0">
					<div class="card-body p-4"><img src="https://cdn.discordapp.com/attachments/533398980428693550/804898333192880128/shop.png" alt="" class="img-fluid d-block mx-auto mb-3">
						<h5> <a href="#" class="text-dark">Dung lượng hàng tồn kho</a></h5>
						<p style="height: 65px;" class="small text-muted font-italic">Nâng cấp dung lượng kho của bạn để có thêm chỗ để lưu trữ sản phẩm của bạn. Sản phẩm của bạn sẽ chỉ được bán khi có người đến cửa hàng của bạn.</p>
						...
						<button onclick="buyUpgrade('stock')" class="btn btn-blue btn-darken-2 white btn-block">Nâng cấp ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(config.market_types.upgrades.stock.price) + `</button>
					</div>
				</div>
			</div>
			...
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card rounded shadow-sm border-0">
					<div class="card-body p-4"><img src="https://cdn.discordapp.com/attachments/533398980428693550/804898331159298078/delivery-truck.png" alt="" class="img-fluid d-block mx-auto mb-3">
						<h5> <a href="#" class="text-dark">Công suất xe tải</a></h5>
						<p style="height: 65px;" class="small text-muted font-italic">Nâng cấp công suất xe tải của bạn để có được một chiếc xe tải tốt hơn để vận chuyển số lượng sản phẩm lớn hơn. Bạn sẽ nhận được số tiền thưởng này khi bạn hoàn thành giao hàng.</p>
						<ul class="small text-muted font-italic">
						...
						<button onclick="buyUpgrade('truck')" class="btn btn-blue btn-darken-2 white btn-block">Nâng cấp ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(config.market_types.upgrades.truck.price) + `</button>
					</div>
				</div>
			</div>
			...
			<div class="col-lg-4 col-md-6 mb-4 mb-lg-0">
				<div class="card rounded shadow-sm border-0">
					<div class="card-body p-4"><img src="https://cdn.discordapp.com/attachments/533398980428693550/804898332371189780/manager.png" alt="" class="img-fluid d-block mx-auto mb-3">
						<h5> <a href="#" class="text-dark">Quan hệ</a></h5>
						<p style="height: 65px;" class="small text-muted font-italic"> Mối quan hệ của bạn với nhà cung cấp càng tốt thì giá mua càng tốt. Bạn sẽ nhận được số tiền khi bắt đầu một công việc mới.</p>
						<ul class="small text-muted font-italic">
						...
						<button onclick="buyUpgrade('relationship')" class="btn btn-blue btn-darken-2 white btn-block">Nâng cấp ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency, maximumFractionDigits: 0, minimumFractionDigits: 0 }).format(config.market_types.upgrades.relationship.price) + `</button>
					</div>
				</div>
			</div>
		`);
					...
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center"><i class="fa fa-plus-circle checkicon"></i>
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 small">
									<div><i class="fas fa-coins"></i></i><span class="ml-2">Số lượng: ` + new Intl.NumberFormat(config.format.location, { style: 'currency', currency: config.format.currency }).format(balance.amount) + `</span></div>
									<div class="ml-3"><i class="fas fa-calendar-alt"></i></i><span class="ml-2">Ngày: ` + timeConverter(balance.date,config.format.location) + `</span></div>
								</div>
							</div>
						</div>
					</li>
