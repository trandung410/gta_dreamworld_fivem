...
AddEventHandler("vrp_stores:getJob",function(key)
	...
	if user_id then
		...
		local sql1 = "SELECT user_id FROM store_business WHERE market_id = @market_id";
		local query_2 = MySQL.Sync.fetchAll(sql1, {['@market_id'] = key})[1];
...
AddEventHandler("vrp_stores:startJob",function(key,id)
	...
	if query.progress == 0 then
		local sql1 = "UPDATE `store_jobs` SET progress = 1 WHERE id = @id";
		MySQL.Sync.execute(sql1, {['@id'] = id});
...
AddEventHandler("vrp_stores:finishContract",function(key,item,truck_level,distance,query_delivery)
	...
	if query_delivery then
		...
		local sql1 = "DELETE FROM `store_jobs` WHERE id = @id;";
		MySQL.Sync.execute(sql1, {['@id'] = query_delivery.id});
	else
	...
	local sql2 = "UPDATE `store_business` SET stock = @stock, goods_bought = goods_bought + @amount, distance_traveled = distance_traveled + @distance WHERE market_id = @market_id";
	MySQL.Sync.execute(sql2, {['@market_id'] = key, ['@stock'] = json.encode(arr_stock), ['@amount'] = amount, ['@distance'] = distance});
end)
...
AddEventHandler("vrp_stores:buyItem",function(key,data)
	...
	if data.amount > 0 then
		...
		if arr_stock[data.item_id] >= data.amount then
			...
			local yrp = xPlayer.getInventoryItem(data.item_id)

			if yrp.limit ~= -1 and (yrp.count + data.amount) > yrp.limit then
				TriggerClientEvent("Notify",source,"negado","<FONT FACE='Montserrat'>Vượt quá số lượng,không thể mua")
			else
				if money >= total_price then
					...
					if query and query[1] then
						...
						local sql1 = "UPDATE `store_business` SET stock = @stock, customers = customers + 1, total_money_earned = total_money_earned + @money WHERE market_id = @market_id";
						MySQL.Sync.execute(sql1, {['@market_id'] = key, ['@money'] = total_price, ['@stock'] = json.encode(arr_stock)});
					end
