	...
	if (item.showmenu){
		...
		for (const contract of contracts) {
					...
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								...
								<div class="ml-3"><i class="fas fa-coins"></i><span class="ml-2">Tiền nhận được: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(contract.reward) + `</span></div>
							</div>
						</div>
					</div>
			...
			list_item += `
						...
						<button onclick="startJob(` + contract.contract_id + `,` + contract.reward + `,` + contract.distance + `)" class="btn btn-blue btn-darken-2 white">Bắt đầu công việc</button>
					</div>
				</li>
			</ul>
			`;
		...
		$('#skills-desc').empty();
		$('#skills-desc').append("Nâng cấp kỹ năng của bạn để hoàn thành công việc tốt hơn (Điểm kỹ năng hiện có: " + users.skill_points + ")");
		...
		list_item = `
			<div class="col-12 mt-3 mb-1">
				<h4 class="text-uppercase">Cửa hàng xe</h4>
				<p>Mua thêm xe tải cho bạn và tài xế của bạn</p>
			</div>`;
			...
			list_item += `
				<div class="card"> <img src="` + truck.img + `" class="card-img-top" width="100%">
					<div class="card-body pt-0 px-0">
						<div class="d-flex flex-row justify-content-between mb-0 mt-3 px-3"> <span class="text-muted">TÊN XE</span>
							<h6>` + truck.name + `</h6>
						</div>
						<hr class="mt-2 mx-3">
						<div class="d-flex flex-row justify-content-between px-3 pb-4">
							<div class="d-flex flex-column"><span class="text-muted">GIÁ BÁN</span></div>
						...
						<div class="mx-3 mt-3 mb-2"><button onclick="buyTruck('` + key + `','` + truck.price + `')" type="button" class="btn btn-blue btn-darken-2 white btn-block"><small>MUA XE</small></button></div> <small class="d-flex justify-content-center text-muted">*Legal Disclaimer</small>
					</div>
				</div>
				`;
		}
		...
		list_item = `
			<div class="col-12 mt-3 mb-1">
				<h4 class="text-uppercase">Cơ quan tuyển dụng</h4>
				<p>Tuyển dụng tài xế mới để làm việc cho công ty của bạn</p>
			</div>
			`;
		for (const driver of drivers) {
			if (driver.user_id == null || driver.user_id == undefined){
				list_item += `
					<div class="card user-card">
						<div class="card-block">
							...
							<p class="text-muted">GIÁ THUÊ: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price_per_km) + `/km</p>
							<hr>
							<p class="text-muted">Loại hàng hoá</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.product_type) + `
							</ul>
							<p class="text-muted">Lái xe đường dài</p>
							<ul class="list-unstyled activity-leval">
								` + getDriverLevelHTML(driver.distance) + `
							</ul>
							<p class="text-muted">Hàng hoá có giá trị</p>
							<ul class="list-unstyled activity-leval">
					...
					<li class="d-flex justify-content-between">
						<div class="d-flex flex-row align-items-center">
							...
							<div class="ml-2">
								...
								<div class="d-flex flex-row mt-1 text-black-50 date-time">
									<div>
										<i class="fas fa-coins"></i><span class="ml-2">Giá thuê: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price) + ` + ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(driver.price_per_km) + `/km</span>
									</div>
									<div class="ml-3">
										<i class="fas fa-medal"></i><span class="ml-2">Kĩ năng: Loại hàng hoá (` + driver.product_type + `) Lái xe đường dài (` + driver.distance + `) Hàng hoá có giá trị (` + driver.valuable + `) Hàng dễ vỡ (` + driver.fragile + `) Giao hàng nhanh (` + driver.fast + `)</span>
									</div>
								</div>
							</div>
						</div>
						<div class="d-flex flex-row align-items-center">
							...
							<button onclick="fireDriver('` + driver.driver_id + `')" class="btn btn-red btn-accent-4 white">Sa thải</button>
						</div>
					</li>
				`);
			}
		...
		$('#loan-title').empty();
		$('#loan-title').append('Nhận các khoản vay để đầu tư vào công ty của bạn (Các khoản vay đang hoạt động tối đa: ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.max_emprestimo) + ')');
		
		$('#loan-1').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[0][0]) + '</h3><span>trả ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[0][1]) + ' một ngày</span>';
		$('#loan-1').append(list_item);
		$('#loan-2').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[1][0]) + '</h3><span>trả ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[1][1]) + ' một ngày</span>';
		$('#loan-2').append(list_item);
		$('#loan-3').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[2][0]) + '</h3><span>trả ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[2][1]) + ' một ngày</span>';
		$('#loan-3').append(list_item);
		$('#loan-4').empty();
		list_item = '<h3 class="success darken-1">' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[3][0]) + '</h3><span>trả ' + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(config.emprestimos[3][1]) + ' một ngày</span>';
				...
				<li class="d-flex justify-content-between">
					<div class="d-flex flex-row align-items-center"><i class="fa fa-check-circle checkicon"></i>
						<div class="ml-2">
							...
							<div class="d-flex flex-row mt-1 text-black-50 date-time">
								<div><i class="fas fa-coins"></i></i><span class="ml-2">Còn lại: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(loan.remaining_amount) + `</span></div>
								<div class="ml-3"><i class="fas fa-coins"></i></i><span class="ml-2">Chi phí hàng ngày: ` + new Intl.NumberFormat(config.formatacao.location, { style: 'currency', currency: config.formatacao.moeda }).format(loan.day_cost) + `</span></div>
							</div>
						</div>
					</div>
					<div class="d-flex flex-row align-items-center">
						<button onclick="payLoan('` + loan.id + `')" class="btn btn-red btn-accent-4 white">Trả khoản vay</button>
					</div>
				</li>
			`;
