...
AddEventHandler('esx_kr_shop:hasEnteredMarker', function(zone)
	...
	elseif zone >= 100 and zone < 200 then
		ShopId = zone
		number = zone
		Msg  = _U('press_to_rob')
	elseif zone >= 200 then
		ShopId = zone
		number = zone
		Msg  = _U('press_to_open_center')
	end
		...
		if ShopId ~= nil then
			...
			DisplayHelpTextFromStringLabel(0, 0, 1, -1)
				...
				if IsControlJustReleased(0, Keys['E']) then
					...
					elseif ShopId >= 100 and ShopId < 200 then
						Robbery(number - 100)
					elseif ShopId >= 200 then
						...
						local elements = {}
							...
							elseif data.current.value == 'takemoney' then
								...
								TriggerServerEvent('esx_kr_shops:takeOutMoney', amount, number - 200)
							...
							elseif data.current.value == 'putmoney' then
								...
								TriggerServerEvent('esx_kr_shops:addMoney', amount, number - 200)
								menu3.close()
									end,
									function(data3, menu3)
								menu3.close()
							...
							ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'sell', {
								...
								TriggerServerEvent('esx_kr_shops:SellShop', number - 200)
								menu4.close()
								end
									end,
									function(data4, menu4)
						...
						elseif data.current.value == 'changename' then
							...
							title = 'What whould you like to name your shop?'
								...
								TriggerServerEvent('esx_kr_shops:changeName', number - 200, data5.value)
								menu5.close()
										end,
										function(data5, menu5)
										menu5.close()
	...
	if showblip then
		...
		for i=1, #IDBLIPS, 1 do
			...
			local blip2 = AddBlipForCoord(vector3(IDBLIPS[i].x, IDBLIPS[i].y, IDBLIPS[i].z))
				...
				BeginTextCommandSetBlipName("STRING")
				AddTextComponentString('Shop ID: ' .. IDBLIPS[i].n)
