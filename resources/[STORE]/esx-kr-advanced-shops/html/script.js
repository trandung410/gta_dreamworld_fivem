			...
			for (i = 0; i < event.data.result.length; i++) {
				...
				$('#wrapper').append(
					`<div class = "image" id = ${event.data.result[i].item} label = ${event.data.result[i].label} count = ${event.data.result[i].count} price = ${event.data.result[i].price}>
						...
						<h5 class = "h4">Hiện có: ${event.data.result[i].count}x</h5>
					</div>`
				);
			}

			$('#wrapper').append(
				` <h6 class = "h4" style = "right: 47.125%; position: absolute;" bottom = ${id - 5}>Made by ICTEAM</h6>`
			)
				...
				var i;
					for (i = 0; i < cb.length; i++) { 
						...
						$('#cart').append(
							...
							<h6>Hiện có: ${cb[i].count}</h4>
							<input type="text" id = ${cb[i].item} count = ${cb[i].count} class = "textareas" placeholder = "Số lượng?"></textarea>
							</div>`
								);
							};
