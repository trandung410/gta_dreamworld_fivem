        ...
        if result == false then 
            ...
            Citizen.CreateThread(function()
                while isCapturing do 
                    ...
                    if timer > 0 then 
                        drawTxt(0.75, 1.44, 1.0, 1.0, 0.4, "<FONT FACE='arial font'>Thời gian chiếm đóng: "..timer, 255, 255, 255, 255)
                    else
...
function drawCharts()
    ...
    if first.name ~= nil then 
        text2 = "<FONT FACE='arial font'>Hạng 1: "..config.JobLabel[first.name].." |Điểm : "..first.point
    end
    if second.name ~= nil then 
        text3 = "<FONT FACE='arial font'>Hạng 2: "..config.JobLabel[second.name].." |Điểm : "..second.point
    end
    if third.name ~= nil then 
        text4 = "<FONT FACE='arial font'>Hạng 3: "..config.JobLabel[third.name].." |Điểm : "..third.point
    end
