...
Citizen.CreateThread(function()
    while true do
        Citizen.Wait(100)
        if IsPlayerDead(PlayerId()) and  isInInventory == true then
            SendNUIMessage({
                    action = "hide"
               })
            ...
            if players[i] ~= PlayerId() then
                ...
                table.insert(
                    elements,
                    {
                        ...
                        player = exports.RufiUniqueID:GetUIDfromID(GetPlayerServerId(players[i]))
                    }
                )
            end
        end
...
RegisterNUICallback(
    ...
    function(data, cb)
        ...
        local player = exports.RufiUniqueID:GetIDfromUID(data.player)
        for i = 1, #players, 1 do
            if players[i] ~= PlayerId() then
        ...
        if foundPlayer then
            ...
            TriggerServerEvent("esx:giveInventoryItem", player, data.item.type, data.item.name, count)
            Wait(250)
            loadPlayerInventory()
        else
            exports.pNotify:SendNotification(
