...
Citizen.CreateThread(function()
  ...
  while true do
    ...
    if nearBankorATM() then
      _sleep = false
      DisplayHelpText("Nhấn ~INPUT_PICKUP~ để truy cập ngân hàng ~b~")
...
AddEventHandler('vb-banking:client:refreshbalance', function(balance)
  ...
  ESX.TriggerServerCallback('vb-banking:server:GetPlayerName', function(playerName)
    SendNUIMessage({
      ...
      playerid = exports.RufiUniqueID:GetUIDfromID(tonumber(_pid))
    })
  end)
end)
...
nearBankorATM = function()
    ...
    local distance = #(vector3(search.x, search.y, search.z) - vector3(_pcoords))
        if distance <= 3 then
          atbank = true
          toreturn = true
          DrawText3D(search.x, search.y, search.z, 'Nhấn ~y~E~w~ để ~r~truy cập~w~ ngân hàng')
