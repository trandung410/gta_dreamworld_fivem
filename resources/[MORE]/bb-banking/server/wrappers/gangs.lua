function loadGangAccount(gangid)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `bank_accounts` WHERE `gangid` = @gang AND `account_type` = 'Gang'", {['@gang'] = self.gid}, function(query)
        if query[1] ~= nil then
    ...
    MySQL.Async.fetchAll("SELECT * FROM `bank_statements` WHERE `account_number` = @ac AND `sort_code` = @sc AND `gangid` = @gid", {['@ac'] = self.accountnumber, ['@sc'] = self.sortcode, ['@gid'] = self.gid}, function(state)
    ...
    self.saveAccount = function()
        MySQL.Async.execute("UPDATE `bank_accounts` SET `amount` = @balance WHERE `record_id` @aid", {['@balance'] = self.balance, ['@aid'] = self.accountid })
    end
...
function createGangAccount(gang, startingBalance)
    ...
    MySQL.Async.fetchAll("SELECT * FROM `bank_accounts` WHERE `gangid` = @gang", {['@gang'] = gang}, function(checkExists)
        if checkExists[1] == nil then
            local sc = math.random(100000,999999)
            local acct = math.random(10000000,99999999)
            MySQL.Async.insert("INSERT INTO `bank_accounts` (`gangid`, `account_number`, `sort_code`, `amount`, `account_type`) VALUES (@gang, @acnum, @sc, @bal, 'Gang')", {['@gang'] = gang, ['@acnum'] = acct, ['@sc'] = sc, ['@bal'] = newBalance }, function(success)
                if success > 0 then
