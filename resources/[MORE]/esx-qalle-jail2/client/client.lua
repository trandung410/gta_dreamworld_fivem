...
function JailLogin()
	...
	ESX.ShowNotification("Bạn Lại Bị Vào Tù!")
...
function UnJail()
	...
	ESX.ShowNotification("Bạn Được Tự Do, Sống Tốt Một Chút! Đừng Lại Bị Tóm, Chúc May Mắn! ")
end
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Đóng Gói"

					if not v["state"] then
						PackageText = "Đang Thực Hiện"
					end
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Bạn Đã Lấy Gói Đồ Này Rồi!")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "[E] Vào/Ra", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "Đóng Gói... " .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "[E] Giao Hàng", 0.4)
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Bỏ Tù Người Chơi Đứng Gần Bạn", value = "jail_closest_player" },
				{ label = "Thả Tự Do Người Chơi", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Thời Gian Sẽ Được Tính Bằng Phút!")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Đến Gần Người Chơi Hơn!")
					else
						ESX.UI.Menu.Open(
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Bạn Cần Ghi Gì Đó Vào Đây!")
						  	else
								menu3.close()
								...
								if closestPlayer == -1 or closestDistance > 3.0 then
								  	ESX.ShowNotification("Lại Gần Người Chơi Hơn!")
								else
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Trong Tù Không Có Ai!")
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Tù Nhân: " .. playerArray[i].name .. " | Thời Gian: " .. playerArray[i].jailTime .. " Phút", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Thả Tự Do Người Chơi",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
