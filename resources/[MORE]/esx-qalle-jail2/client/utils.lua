...
function TeleportPlayer(pos)
	...
	if #Values["goal"] > 1 then
		...
		function(data, menu)
			...
			if action == "Jail" or action == "Security" then --action == "Boiling Broke" or 

				if PlayerData.job.name ~= "police" then
					ESX.ShowNotification("Bạn Không Có Chìa Khóa!")
					return
				end
			end
...
Citizen.CreateThread(function()
	...
	AddTextComponentString('Nhà Tù Dream World')
    --AddTextComponentString('Khu biet giam')
