	...
	if (xPlayer["job"]["name"] == "police") then
		...
		if GetPlayerName(jailPlayer) ~= nil then
			...
			if jailTime ~= nil then
				...
				TriggerClientEvent("esx:showNotification", src, GetPlayerName(jailPlayer) .. " Bỏ Tù Trong " .. jailTime .. " Phút !")

				if args[3] ~= nil then
					GetRPName(jailPlayer, function(Firstname, Lastname)
						TriggerClientEvent('chat:addMessage', -1, { args = { "TUYÊN ÁN",  Firstname .. " " .. Lastname .. " Bị Bỏ Tù Vì Tội: " .. args[3] }, color = { 249, 166, 0 } })
					end)
				end
			else
			...
			TriggerClientEvent("esx:showNotification", src, "ID Này Không Online!")
		end
	else
		TriggerClientEvent("esx:showNotification", src, "Bạn Không Phải Sĩ Quan Cảnh Sát !")
	end
end)
...
AddEventHandler("esx-qalle-jail:jailPlayer", function(targetSrc, jailTime, jailReason)
	...
	GetRPName(targetSrc, function(Firstname, Lastname)
		TriggerClientEvent('chat:addMessage', -1, { args = { "TUYÊN ÁN",  Firstname .. " " .. Lastname .. " Bị Bỏ Tù Vì Tội: " .. jailReason }, color = { 249, 166, 0 } })
	end)

	TriggerClientEvent("esx:showNotification", src, GetPlayerName(targetSrc) .. " Bỏ Tù Trong " .. jailTime .. " Phút !")
...
AddEventHandler("esx-qalle-jail:unJailPlayer", function(targetIdentifier)
	...
	TriggerClientEvent("esx:showNotification", src, xPlayer.name .. " Trả Tự Do !")
end)
...
AddEventHandler("esx-qalle-jail:prisonWorkReward", function()
	...
	TriggerClientEvent("esx:showNotification", src, "Cảm Ơn, Ở Đây Có Một Chút Tiền Để Mua Đồ Ăn!")
end)
