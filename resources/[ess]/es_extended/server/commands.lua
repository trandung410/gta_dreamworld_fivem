...
TriggerEvent('es:addGroupCommand', 'setmoney', 'superadmin', function(source, args, user)
	...
	local xPlayer = ESX.GetPlayerFromId(exports.RufiUniqueID:GetIDfromUID(target))
...
TriggerEvent('es:addGroupCommand', 'clearinventory', 'admin', function(source, args, user)
	...
	if args[1] then
		...
		xPlayer = ESX.GetPlayerFromId(exports.RufiUniqueID:GetIDfromUID(tonumber(args[1])))
...
ESX.RegisterCommand('createteam', 'user', function(xPlayer, args, showError)
	xPlayer.createTeam()
end, false, {help = 'Tạo tổ đội'})

ESX.RegisterCommand('jointeam', 'user', function(xPlayer, args, showError)
	xPlayer.joinTeam(args.teamID)
	--ESX.JoinTeam(args.teamID, xPlayer.source)
end, true, {help = 'Gia nhập tổ đội', validate = true, arguments = {
	{name = 'teamID', help = 'ID của đội muốn gia nhập', type = 'number'}
}})

ESX.RegisterCommand('leaveteam', 'user', function(xPlayer, args, showError)
	xPlayer.leaveTeam()
end, false, {help = 'Rời tổ đội'})

ESX.RegisterCommand('invitetoteam', 'user', function(xPlayer, args, showError)
	xPlayer.inviteToTeam(args.playerID.source)
	--ESX.JoinTeam(args.teamID, xPlayer.source)
end, true, {help = 'Mời người chơi vào tổ đội', validate = true, arguments = {
	{name = 'playerID', help = 'ID của người chơi bạn muốn mời', type = 'player'}
}})

ESX.RegisterCommand('kickteam', 'user', function(xPlayer, args, showError)
	xPlayer.kickFromTeam(args.playerID.source)
	--ESX.JoinTeam(args.teamID, xPlayer.source)
end, true, {help = 'Đá người chơi khỏi tổ đội', validate = true, arguments = {
	{name = 'playerID', help = 'ID của người chơi bạn muốn Đá', type = 'player'}
}})