...
$(function() {
		window.addEventListener('message', function(event) {
			...
			$('#wrapper').append(
				` <h6 class = "h4" style = "right: 47.125%; position: absolute;" bottom = ${id - 5}>VOZ</h6>`
			)
			...
			$("body").on("click", "#buybutton", function() {
				...
				if(stored == '1'){
					...
					$.post('http://esx_advancedgarage/spawnVehicle', JSON.stringify({vehicle : event.data.result[number].value, plate : plate}));
					$.post('http://esx_advancedgarage/escape', JSON.stringify({}));
					location.reload(true);
					$('#wrapper').hide("fast");
				}else{
