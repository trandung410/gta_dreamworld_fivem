...
AddEventHandler('HRP:Impound:VehicleUnimpounded', function (data, index)
	...
	_ESX.ShowNotification("<FONT FACE='Montserrat'>Xe của bạn biển số :  " .. data.plate .. " đã được thả!")
...
AddEventHandler('HRP:Impound:CannotUnimpound', function ()
	_ESX.ShowNotification("<FONT FACE='Montserrat'>Xe của bạn không thể được thả lúc này vị bạn không có đủ tiền");
end)
...
RegisterNUICallback('impound', function(data, cb)
	...
	--TriggerServerEvent('lorraxs:forceDelete', ObjToNet(_ESX.Game.GetClosestVehicle()))
