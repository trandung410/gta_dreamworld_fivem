...
ESX.RegisterServerCallback('esx_advancedgarage:getOwnedAircrafts', function(source, cb)
	...
	if Config.DontShowPoundCarsInGarage == true then
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job AND `stored` = @stored AND `impound` = 0', {
			['@owner']  = xPlayer.identifier,
			['@Type']   = 'aircraft',
			['@job']    = '',
			['@stored'] = true
		...
		MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND Type = @Type AND job = @job  AND `impound` = 0', {
			['@owner']  = xPlayer.identifier,
			['@Type']   = 'aircraft',
			['@job']    = ''
		}, function(data)
...
ESX.RegisterServerCallback('esx_advancedgarage:getOutOwnedPolicingCars', function(source, cb)
	...
	local xPlayer = ESX.GetPlayerFromId(source)
	MySQL.Async.fetchAll('SELECT * FROM owned_vehicles WHERE owner = @owner AND job = @job AND `stored` = @stored  AND `impound` = 0', {
		['@owner'] = xPlayer.identifier,
		['@job']    = 'police',
		['@stored'] = false
	}, function(data) 
...
RegisterNetEvent("garage:server:removeOldVehicle")
AddEventHandler("garage:server:removeOldVehicle", function(plate)
