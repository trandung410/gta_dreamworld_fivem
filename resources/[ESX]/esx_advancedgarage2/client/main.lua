	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
			...
			for _,v in pairs(ownedCars) do
				if v.vehicle.model ~= -520352756 then
...
RegisterNUICallback('spawnVehicle', function(data)
	...
	local dumped = ESX.DumpTable(data.vehicle)
	print(dumped)
	SpawnVehicle(data.vehicle, data.plate)

end)
RegisterNUICallback('spawnVehicle1', function(data)
	--vehicle = json.decode(data.vehicle)
	local dumped = ESX.DumpTable(data.vehicle)
	print(dumped)
	SpawnPoundedVehicle(data.vehicle, data.plate)
...
function SpawnVehicle(vehicle, plate)
	...
	TriggerServerEvent("garage:server:removeOldVehicle", plate)
