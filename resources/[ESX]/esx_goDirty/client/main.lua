...
Citizen.CreateThread(function()
	...
	local dupm = ESX.DumpTable(PlayerData)
	print(dupm)
end)
--------------------------------------------------------------------------------
-- NE RIEN MODIFIER
...
function MenuVehicleSpawner()
	...
	ESX.UI.Menu.Open(
		...
		function(data, menu)
			ESX.TriggerServerCallback('esx_goDirty:checkPolice', function(result)
				if result then 
					ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 270.0, function(vehicle)
						...
						xPlayer.removeAccountMoney('black_money', '5000') -- Cost $500 Dirty Money
					...
					ESX.ShowNotification("Không đủ ~r~cảnh sát ~w~trong thành phố")
				end
			end)
