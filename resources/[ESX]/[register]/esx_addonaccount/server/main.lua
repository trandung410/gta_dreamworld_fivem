...
AddEventHandler("esx_addonaccount:registerGang", function(name, label)
	MySQL.Async.execute("INSERT INTO addon_account (name, label, shared) VALUES (@name, @label, 1)", {
		['@name'] = "society_"..name,
		['@label'] = label
	}, function(rowChanged)
		MySQL.Sync.execute('INSERT INTO addon_account_data (account_name, money, owner) VALUES (@account_name, @money, NULL)', {
		...
		money = 0
		local addonAccount   = CreateAddonAccount("society_"..name, nil, money)
		SharedAccounts["society_"..name] = addonAccount
	end)
end)