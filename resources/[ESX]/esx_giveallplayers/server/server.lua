...
TriggerEvent('es:addGroupCommand', 'giveallitem', 'superadmin', function(source, args, user)
...
TriggerEvent('es:addGroupCommand', 'giveallweapon', 'superadmin', function(source, args, user)
	if Config.AllowAdminInGameCommand then
		if source ~= 0 then
			local _source = source
			local xPlayers = ESX.GetPlayers()
...
TriggerEvent('es:addGroupCommand', 'addallmoney', 'superadmin', function(source, args, user)
	if Config.AllowAdminInGameCommand then
		if source ~= 0 then
			local _source = source
			local xPlayers = ESX.GetPlayers()
