...
Citizen.CreateThread(function()
	AddTextEntry("ParaTattoos", "<FONT FACE='Montserrat'>Tiệm Xăm")
...
function CreateScale(sType)
	if scaleString ~= sType and sType == "Mở Tiệm Tattoo's" then
		scaleType = setupScaleform("instructional_buttons", "Mở Tiệm Tattoo's", 38)
		scaleString = sType
	elseif scaleString ~= sType and sType == "Control" then
		scaleType = setupScaleform2("instructional_buttons", "Đổi Góc Nhìn", 21, "Độ Mờ Của Mực", {90, 89}, "Đồng Ý/Hủy Xăm", 191)
		scaleString = sType
	end
end

Citizen.CreateThread(function()
	JayMenu.CreateMenu("tattoo", "<FONT FACE='Montserrat'>Tiệm Xăm", function()
        return CloseTattooShop()
    end)
    JayMenu.SetSubTitle('tattoo', "Thể Loại")
			...
			for _,interiorId in ipairs(Config.interiorIds) do
				if GetInteriorFromEntity(PlayerPedId()) == interiorId then
					...
					if not IsPedInAnyVehicle(PlayerPedId(), false) then
						...
						Draw3DText("Nhấn ~g~[E]~y~ để xăm hình")
