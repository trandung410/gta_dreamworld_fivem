...
local safetime =math.floor(math.random(3,12))
...
function reloadServerData()
    ...
    for key,val in ipairs(res) do
        ...
        if val.owner~=nil and not namecache[val.owner] then namecache[val.owner]=MySQL.Sync.fetchAll("SELECT firstname,lastname FROM users WHERE identifier = @identifier",{["@identifier"]=val.owner})[1] end
        dataCache[val.id] = {
            id = val.id,
            name = val.name,
            address = val.address,
            ...
            owner = val.owner,
            owner_name = val.owner~=nil and namecache[val.owner].name or "",
	...
	for i = 1 , #res, 1 do
		if res[i].time > 0 then
			--TriggerClientEvent('mythic_notify:client:SendAlert', -1, { type = 'error', text = val.time})
			ESX.SetTimeout(3600000, function()
				demnguoc(res[i].id)
			end)
		end
	end
...
function addMoney(identifier,money,business)
    ...
    if source ~= nil then
        ...
        MySQL.Sync.execute('UPDATE `user_accounts` SET `money` = `money` + @bank WHERE `identifier` = @identifier',{['@bank'] = money, ['@identifier'] = identifier})
    end
end
...
ESX.RegisterServerCallback("esx_business:buyBusiness", function(source,cb,business)
    local xPlayer = ESX.GetPlayerFromId(source)
    local identifier = xPlayer.getIdentifier()
    if xPlayer.job.name == "HongHung" then
        if dataCache[business]["owner"]==nil then
            if xPlayer.getMoney()>=10000 then
                ...
                MySQL.Sync.execute('UPDATE businesses SET owner = @identifier, time = @time, stock = 100, icon = 497 WHERE id = @business', {["@identifier"] = identifier, ["@time"] = math.floor(math.random(3,12)), ["@business"] = tonumber(business)}, nil)
                Citizen.CreateThread(function() reloadServerData();reloadPlayersData() end)
                cb(0)
            else
                cb(2)
    ...
    elseif xPlayer.job.name == "GSgang" then
        if dataCache[business]["owner"]==nil then
            if xPlayer.getMoney()>=10000 then
                dataCache[business]["owner"]=identifier -- counteract the loading time from db (player can accidentally buy twice)
                xPlayer.removeMoney(10000)
                MySQL.Sync.execute('UPDATE businesses SET owner = @identifier, time = @time, stock = 100, icon = 484 WHERE id = @business', {["@identifier"] = identifier, ["@time"] = math.floor(math.random(3,12)), ["@business"] = tonumber(business)}, nil)
                Citizen.CreateThread(function() reloadServerData();reloadPlayersData() end)
                cb(0)
            else
                cb(2)
    ...
    elseif xPlayer.job.name == "biker" then 
        if dataCache[business]["owner"]==nil then
            if xPlayer.getMoney()>=10000 then
                dataCache[business]["owner"]=identifier -- counteract the loading time from db (player can accidentally buy twice)
                xPlayer.removeMoney(10000)
                MySQL.Sync.execute('UPDATE businesses SET owner = @identifier, time = @time, stock = 100, icon = 559 WHERE id = @business', {["@identifier"] = identifier, ["@time"] = math.floor(math.random(3,12)), ["@business"] = tonumber(business)}, nil)
                Citizen.CreateThread(function() reloadServerData();reloadPlayersData() end)
                cb(0)
            else
                cb(2)
    ...
    elseif xPlayer.job.name == "cartel" then 
        if dataCache[business]["owner"]==nil then
            if xPlayer.getMoney()>=10000 then
                dataCache[business]["owner"]=identifier -- counteract the loading time from db (player can accidentally buy twice)
                xPlayer.removeMoney(10000)
                MySQL.Sync.execute('UPDATE businesses SET owner = @identifier, time = @time, stock = 100, icon = 150 WHERE id = @business', {["@identifier"] = identifier, ["@time"] = math.floor(math.random(3,12)), ["@business"] = tonumber(business)}, nil)
                Citizen.CreateThread(function() reloadServerData();reloadPlayersData() end)
                cb(0)
            else
                cb(2)
    ...
    elseif xPlayer.job.name == "gangster" then 
        if dataCache[business]["owner"]==nil then
            if xPlayer.getMoney()>=10000 then
                dataCache[business]["owner"]=identifier -- counteract the loading time from db (player can accidentally buy twice)
                xPlayer.removeMoney(10000)
                MySQL.Sync.execute('UPDATE businesses SET owner = @identifier, time = @time, stock = 100, icon = 226 WHERE id = @business', {["@identifier"] = identifier, ["@time"] = math.floor(math.random(3,12)), ["@business"] = tonumber(business)}, nil)
                Citizen.CreateThread(function() reloadServerData();reloadPlayersData() end)
                cb(0)
            else
                cb(2)
    ...
    elseif xPlayer.job.name == "mafia" then 
        if dataCache[business]["owner"]==nil then
            if xPlayer.getMoney()>=10000 then
                dataCache[business]["owner"]=identifier -- counteract the loading time from db (player can accidentally buy twice)
                xPlayer.removeMoney(10000)
                MySQL.Sync.execute('UPDATE businesses SET owner = @identifier, time = @time, stock = 100, icon = 269 WHERE id = @business', {["@identifier"] = identifier, ["@time"] = math.floor(math.random(3,12)), ["@business"] = tonumber(business)}, nil)
                Citizen.CreateThread(function() reloadServerData();reloadPlayersData() end)
                cb(0)
            else
                cb(2)
        ...
        if dataCache[business]["owner"]==nil then
            if xPlayer.getMoney()>=10000 then
                dataCache[business]["owner"]=identifier -- counteract the loading time from db (player can accidentally buy twice)
                xPlayer.removeMoney(10000)
                MySQL.Sync.execute('UPDATE businesses SET owner = @identifier, time = @time, stock = 100, icon = 491 WHERE id = @business', {["@identifier"] = identifier, ["@time"] = math.floor(math.random(3,12)), ["@business"] = tonumber(business)}, nil)
                Citizen.CreateThread(function() reloadServerData();reloadPlayersData() end)
                cb(0)
            else
                cb(2)
...
ESX.RegisterServerCallback("esx_business:createBusiness", function(source,cb,business)
    ...
    if xPlayer.getGroup()=="superadmin" then
        ...
        MySQL.Async.execute("INSERT INTO businesses(id,name,address,description,blipname,price,earnings,position,stock_price,taxrate,employees,time) VALUES (NULL,@name,@address,@description,@blipname,@price,@earnings,@position,@stock_price,@taxrate,'{}',0)", {
            ["@name"] = business["name"],
...
RegisterServerEvent("esx_business:batdauchiemdong")
AddEventHandler("esx_business:batdauchiemdong",function(ten)
    TriggerClientEvent('lorraxsnotify:thongbao', -1, {type = 'error', text = "Khu vực "..ten.." đang bị chiếm đóng bởi "..GetPlayerName(source)})
end)
...
function demnguoc(id)
	local id = id
	local thoigian=MySQL.Sync.fetchAll("SELECT time,name FROM businesses WHERE id = @name",{["@name"]=id})[1]
	--local dumpedTable = ESX.DumpTable(thoigian)
	--print(dumpedTable)
	--TriggerClientEvent('mythic_notify:client:SendAlert', -1, { type = 'error', text = thoigian.time})
	if thoigian.time >= 1 then 
		thoigian.time = thoigian.time -1 
		MySQL.Sync.execute('UPDATE businesses SET time = @time WHERE id = @business', {["@business"] = tonumber(id), ["@time"] = thoigian.time}, nil)		
		ESX.SetTimeout(3600000, function()
			demnguoc(id)
		end)
	else
		MySQL.Sync.execute('UPDATE businesses SET owner = NULL, time = 0, icon = 491 WHERE id = @business', {["@business"] = tonumber(id)}, nil)
        reloadServerData();reloadPlayersData()
        TriggerClientEvent('lorraxsnotify:thongbao', -1, {type = 'error', text = "Khu vực "..thoigian.name.." đã hết thời gian an toàn, có thể chiếm đóng"})
	end
end