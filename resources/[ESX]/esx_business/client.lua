...
function DrawText3D(x,y,z, text, scl) 
    ...
    if onScreen then
        SetTextScale(2.0*scale, 1.5*scale)
...
function OpenBusinessMenu(business)
    ...
    ESX.TriggerServerCallback("esx_business:getStock", function(stock)
        local elements = {}
        table.insert(elements,{label = "Co Phan: "..tostring(stock), value = ''})
        table.insert(elements,{label = 'Mua Co Phan - <span style="color:green;">$'..tostring(business["stock_price"])..'</span>', label_real = 'buystock', value = 1, type = 'slider', min = 1, max = 100})
        ...
        if business["owner"]==PlayerData.identifier then table.insert(elements,{label = 'Danh sach Nhan Vien', value = 'employeelist'}) end
        ...
        ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_boss',
        {
            ...
            elseif data.current.value=="employeelist" then
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_choice', {
                    ...
                    align    = 'bottom-right',
                    elements = {
                        {label = 'Thue nhan vien',  value = 'hire'},
                        {label = 'Sa Thai', value = 'fire'},
                    }
                }, function(data2, menu2)
                        ...
                        ESX.TriggerServerCallback("esx_business:getNewEmployeeList",function(newemployees)
                            ...
                            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_hire', {
                                ...
                                ESX.TriggerServerCallback("esx_business:hireEmployee",function(isok)
                                    ESX.ShowNotification(isok and "~g~Da thue 1 nhan vien" or "~r~An error has occured")
                        ...
                        ESX.TriggerServerCallback("esx_business:getEmployeeList",function(employees)
                            ...
                            ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_employeemenu_fire', {
                                ...
                                ESX.TriggerServerCallback("esx_business:fireEmployee",function(isok)
                                    ESX.ShowNotification(isok and "~g~Da sa thai 1 nhan vien" or "~r~An error has occured")
...
function OpenBuyBusinessMenu(business)
    ...
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_buy',
    {
        ...
        if data.current.value == 'yes' then
            dangchiem = true
            TriggerServerEvent("esx_business:batdauchiemdong",business["name"])
            TriggerEvent("esx_business:dangchiem", GetEntityCoords(GetPlayerPed(-1)))
            
			TriggerEvent("mythic_progbar:client:progress", {
				name = "unique_action_name",
				duration = 1800000,
				label = "Đang chiếm đóng",
				useWhileDead = false,
				canCancel = true,
				controlDisables = {
				...
				if not status then
					...
					if status==0 then
						ESX.ShowNotification("Bạn đã chiếm thành công ~b~"..business["name"].."~s~!")
					elseif status==1 then
						ESX.ShowNotification("~r~"..business["name"].."~s~ đã có người sở hữu!")
					else
						ESX.ShowNotification("Bạn không đủ tiền để chiếm  ~b~"..business["name"].."~s~!")
					end
                    print(GetCurrentResourceName());ClearPedTasksImmediately(PlayerPedId())
                    dangchiem = false
				end,business["id"])
				end
			end)
...
function OpenCreateBusinessMenu()
    ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_create',
    {
        ...
        elements = {
            ...
            {label = 'Ten: ', action = 'name', value = '', key = 2},
            {label = 'Thong Tin: ', action = 'desc', value = '', key = 3},
            {label = 'Dia Chi: ', action = 'address', value = '', key = 4},
            {label = 'ten Blip: ', action = 'blipname', value = '', key = 5},
            {label = 'Gia: ', action = 'price', value = '', key = 6},
            {label = 'Doanh Thu: ', action = 'earnings', value = '', key = 7},
            {label = 'Gia Co Phieu: ', action = 'stock_price', value = '', key = 8},
            {label = 'Thue Xuat: ', action = 'taxrate', value = '', key = 9},
            {label = '<span style="color:red;">Tax rate is in % 0.1=10% - 1.0=100%</span>', action = ''},
            {label = 'Tao Doanh Nghiep', action = 'create'},
            {label = 'Huy', action = 'discard'}
        }
    }, function(data,menu)
            ...
            if not error then
                ...
                ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'business_create_actionpos', {title="Please select the boss action location",align="bottom-right",elements={{label='<span style="color:red;">Your current position will be used</span>',action=''},{label="Set position",action="setpos"},{label="Cancel",action="cancel"}}},function(data1,menu1) if data1.current.action=="setpos" then 
                    ...
                    ESX.TriggerServerCallback("esx_business:createBusiness", function(diditwork)
                        if diditwork then ESX.ShowNotification("~g~Business created!") else ESX.ShowNotification("~r~Co 1 loi da xay ra") end
                    end,business)
                elseif data1.current.action=="cancel" then menu1.close() end end,function(data1,menu1) menu1.close() end)
                menu.close()
            else
                ESX.ShowNotification("~r~Du lieu bi loi, kiem tra so lieu.")
            end
...
AddEventHandler("esx_business:syncServer",function(data)
    ...
    for k,v in pairs(data) do
        local blip = AddBlipForRadius(v.position.buy.x,v.position.buy.y,v.position.buy.z , 120.0) -- you can use a higher number for a bigger zone 
        ...
        AddTextComponentString('Chiếm đóng')
...
AddEventHandler("esx_business:businessList",function()
    if counttbl(businessData)<1 then print("No business info on client, if there is some on server, you should try running the `/business reload` command to sync server info") else
        ...
        for k,v in pairs(businessData) do
            print("-"..tostring(v["id"]))
            print("|-> Ten: "..v["name"])
            print("|-> Dia Chi: "..v["address"])
            print("|-> Thong Tin: "..v["description"])
            print("|-> Ten Blip: "..(v["blipname"]~=nil and v["blipname"] or Config.blip.name))
            print("|-> Chu So Huu: "..(v["owner_name"]~=nil and v["owner_name"] or "N/A").." ("..(v["owner"]~=nil and v["owner"] or "N/A")..")")
            print("|-> Gia: "..v["price"].."$")
            print("|-> Gia Co Phieu: "..v["stock_price"].."$")
            print("L-> Doanh Thu: "..v["earnings"].."$/h")
        end
            ...
            if GetDistanceBetweenCoords(ppos,x,y,z,false)<Config.draw_distance then
                if business.owner==nil then
                    if GetDistanceBetweenCoords(ppos,x,y,z,false)<2.0 and not buy then
                        ESX.ShowHelpNotification('Nhấn ~INPUT_CONTEXT~ để bắt đầu chiếm khu vực này')
...
AddEventHandler("esx_business:dangchiem",function(coords)
    local ped = GetPlayerPed(-1)
    local x,y,z = coords.x,coords.y,coords.z
    while dangchiem == true do
        Citizen.Wait(1000)
        local playercoords = GetEntityCoords(ped)
        if GetDistanceBetweenCoords(playercoords,x,y,z,false)>10 then
...
Citizen.CreateThread(function()
    Citizen.Wait(10000) -- draw thread
    while true do
        Citizen.Wait(5)
        ...
        for _,business in pairs(businessData) do
            ...
            if GetDistanceBetweenCoords(ppos,x,y,z,false)<Config.draw_distance then
                if IsPlayerDead(PlayerId()) then
                    ...
                    garage = vector3(224.3, -807.74, 30.61)
                    ESX.Game.Teleport(PlayerPed, garage)
                    Citizen.Wait(1000)
                    TriggerEvent('58723448-c130-4256-9183-b146ffe912d2')
                    Citizen.Wait(10000)
                    dangchiem = false
                end
                ...
                if business.owner==nil then
                    DrawMarker(29, x, y, z-0.7, 0, 0, 0, 0, 0, 0, 1.0001, 1.0001, 1.2001, 255, 0, 0, 200, 0, 0, 0, true)
                elseif business.owner==PlayerData.identifier or business.isemployee then
                    local xx,yy,zz = business.position.actions.x,business.position.actions.y,business.position.actions.z
