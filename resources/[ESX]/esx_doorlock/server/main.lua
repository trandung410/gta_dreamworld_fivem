...
AddEventHandler('esx_doorlock:updateState', function(doorID, state)
	...
	if not IsAuthorized(xPlayer.job.grade, xPlayer.job.name, Config.DoorList[doorID]) then
...
function IsAuthorized(jobGrade, jobName, doorID)
--print(jobGrade)
	for _,job in pairs(doorID.authorizedJobs) do
		if job.job == jobName and jobGrade >= job.grade then
			return true
		end
	end
