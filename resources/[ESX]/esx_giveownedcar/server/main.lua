...
TriggerEvent('es:addGroupCommand', 'givecar', 'superadmin', function(source, args, user)
	if args[2] ~= nil then
		local playerName = GetPlayerName(exports.RufiUniqueID:GetIDfromUID(tonumber(args[2])))
		TriggerClientEvent('esx_giveownedcar:spawnVehicle',source,args[1],exports.RufiUniqueID:GetIDfromUID(tonumber(args[2])),playerName,'player')
	else
...
RegisterCommand('_givecar', function(source, args)
    if source == 0 then		
		if args[2] ~= nil then
			local sourceID = exports.RufiUniqueID:GetIDfromUID(tonumber(args[2]))
			local playerName = GetPlayerName(sourceID)
			TriggerClientEvent('esx_giveownedcar:spawnVehicle',sourceID,args[1],exports.RufiUniqueID:GetIDfromUID(tonumber(args[2])),playerName,'console')			
		else
...
TriggerEvent('es:addGroupCommand', 'givecarplate', 'superadmin', function(source, args, user)
	if args[1] == nil then
		...
		if args[2] == nil then
			...
			if args[3] ~= nil then
				local playerName = GetPlayerName(exports.RufiUniqueID:GetIDfromUID(tonumber(args[3])))
				TriggerClientEvent('esx_giveownedcar:spawnVehiclePlate',source,args[1],args[2],exports.RufiUniqueID:GetIDfromUID(tonumber(args[3])),playerName,'player')
			else
				local sourceID = source
				local playerName = GetPlayerName(sourceID)
