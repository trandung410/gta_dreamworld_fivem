...
Citizen.CreateThread(function()
  ...
  while true do
	...
	if (IsControlJustPressed(1, 303)) then
		...
		notowned = 0
		if #cars == 0 then
			ESX.ShowNotification("Không có phương tiền để khoá gần đó.")
		else
			for j=1, #cars, 1 do
				...
				ESX.TriggerServerCallback('carlock:isVehicleOwner', function(owner)
					if owner and hasAlreadyLocked ~= true then
						...
						if lock == 1 or lock == 0 then
							...
							ESX.ShowNotification('Bạn đã ~r~khoá~s~ xe của bạn ~y~'..vehicleLabel..'~s~.')
						...
						elseif lock == 2 then
							...
							ESX.ShowNotification('Bạn đã ~g~Mở khoá~s~ xe của bạn ~y~'..vehicleLabel..'~s~.')
							if not IsPedInAnyVehicle(PlayerPedId(), true) then
								TaskPlayAnim(PlayerPedId(), dict, "fob_click_fp", 8.0, 8.0, -1, 48, 1, false, false, false)
							end
							hasAlreadyLocked = true
					...
					if notowned == #carstrie then
						ESX.ShowNotification("Không có phương tiện ở gần đây.")
					end	
				end, plate)
			end			
		end
...
AddEventHandler('esx_carlock:lockEvent', function()
	local coords = GetEntityCoords(GetPlayerPed(-1))
		local hasAlreadyLocked = false
		local cars = ESX.Game.GetVehiclesInArea(coords, 30)
		print(#cars)
