	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedCars', function(ownedCars)
		if #ownedCars == 0 then
	        ...
	        TriggerEvent("pNotify:SendNotification",  {
				text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Bạn không sở hữu một chiếc xe nào",
		        type = "error",
		        timeout = 5000,
		        layout = "topRight",
	        })
			...
			for _,v in pairs(ownedCars) do
				...
				--[[ 	local vehBodyDamage	   = math.floor(v.vehicle.bodyHealth) ]]
					local labelvehicle
...
--[[ 				else
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_car', {
			...
			if data.current.value.stored then
				...
				TriggerServerEvent("discordbot:addcargarage", data.current.value.plate)
			else
				--ESX.ShowNotification(_U('car_is_impounded'))
				TriggerEvent("pNotify:SendNotification",  {
					text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Xe của bạn đang bị giam.",
		            type = "info",
		            timeout = 5000,
		            layout = "topRight",
	            })
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedBoats', function(ownedBoats)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_boat', {
			...
			if data.current.value.stored then
				...
				TriggerEvent("pNotify:SendNotification",  {
					text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Thuyền của bạn đang bị giam",
		            type = "info",
		            timeout = 5000,
		            layout = "topRight",
	            })
	...
	ESX.TriggerServerCallback('esx_advancedgarage:getOwnedAircrafts', function(ownedAircrafts)
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_owned_aircraft', {
			...
			if data.current.value.stored then
				...
				TriggerEvent("pNotify:SendNotification",  {
					text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Máy bay của bạn đang bị giam.",
		            type = "info",
		            timeout = 5000,
		            layout = "topRight",
	            })
...
function StoreOwnedCarsMenu()
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('esx_advancedgarage:storeVehicle', function(valid)
			...
			--	end	
				TriggerServerEvent("discordbot:delcargarage", plate)
			else
				--ESX.ShowNotification(_U('cannot_store_vehicle'))
				TriggerEvent("pNotify:SendNotification",  {
					text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Bạn không thể giữ phương tiện này.",
		            type = "error",
		            timeout = 5000,
		            layout = "topRight",
	            })
		...
		TriggerEvent("pNotify:SendNotification",  {
			text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Không có xe để lưu trữ trong nhà để xe.",
		    type = "error",
		    timeout = 5000,
		    layout = "topRight",
	    })
	...
	if IsPedInAnyVehicle(playerPed,  false) then
		...
		ESX.TriggerServerCallback('esx_advancedgarage:storeVehicle', function(valid)
			if valid then
				...
				--ESX.ShowNotification(_U('cannot_store_vehicle'))
				TriggerEvent("pNotify:SendNotification",  {
					text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Bạn không thể lưu trữ phương tiện này.!",
		            type = "error",
		            timeout = 5000,
		            layout = "topRight",
	            })	
		...
		ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'return_owned_car', {
			...
			ESX.TriggerServerCallback('esx_advancedgarage:checkMoneyCars', function(hasEnoughMoney)
				if hasEnoughMoney then
					...
					TriggerServerEvent("discordbot:poundcargarage", data.current.value.plate)
				else
					--ESX.ShowNotification(_U('not_enough_money'))
					TriggerEvent("pNotify:SendNotification",  {
		                text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Bạn không có đủ tiền!",
		                type = "error",
		                timeout = 5000,
		                layout = "topRight",
	                })
...
function reparation(apprasial, vehicle, vehicleProps)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'delete_menu', {
		...
		elseif data.current.value == 'no' then
			...
			TriggerEvent("pNotify:SendNotification",  {
                text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Đến gặp kỹ thuật viên hoặc tự sửa chữa.",
		        type = "info",
		        timeout = 5000,
		        layout = "topRight",
	        })
...
function putaway(vehicle, vehicleProps)
	...
	TriggerEvent("pNotify:SendNotification",  {
        text = "<span style='font-size:14;font-weight: 900'>Garage</span><br>Xe của bạn được cất giữ trong nhà để xe.",
		type = "info",
		timeout = 5000,
		layout = "topRight",
	})
...
function SpawnVehicle(vehicle, plate)
    --exports['pogressBar']:drawBar(1200, 'Wait...')
