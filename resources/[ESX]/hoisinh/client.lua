...
AddEventHandler("knb:mech", function()
    ...
    if DoesEntityExist(targetVeh) then
		...
		exports['mythic_notify']:DoHudText('error', 'Không thể được sử dụng vì có đủ bác sĩ.')
		else
...
function GoToTarget(x, y, z, vehicle, driver, vehhash, player)
    ...
    ShowAdvancedNotification(companyIcon, companyName, "<FONT FACE = 'Tahoma'>Bác sĩ thông báo", "Một bác sĩ đã được gửi đến khu vực của bạn. ~y~")
    enroute = true
    while enroute do
        Citizen.Wait(500)
...
function RepairVehicle(player, vehicle, driver)
    ...
    Citizen.Wait(500)
	ShowAdvancedNotification(mechPedPick.icon, mechPedPick.name, "<FONT FACE = 'Tahoma'>Yasuo" , mechPedPick.lines[math.random(#mechPedPick.lines)])
