...
RegisterNetEvent('pk_bee:pic')
AddEventHandler('pk_bee:pic', function ()
		...
		local x = math.random(1,Config.deleteobject)
			...
			for i=1, #Pop1, 1 do
				...
				Detehand()
				Citizen.Wait(100)
				TriggerServerEvent('pk_bee:pickedUp')
...
Citizen.CreateThread(function()
	while true do
		...
		if nearbyObject and IsPedOnFoot(playerPed) then
			ESX.ShowHelpNotification("Nhấn ~r~[E]~s~ để lấy ~g~mật ong")
			if IsDisabledControlJustReleased(0, 38) then
				TriggerServerEvent('pk_bee:Check')
