...
RegisterServerEvent('pk_bee:pickedUp')
AddEventHandler('pk_bee:pickedUp', function()
...
RegisterServerEvent('pk_bee:Check')
AddEventHandler('pk_bee:Check', function()
    local _source = source
    local xPlayer = ESX.GetPlayerFromId(_source)
    local item = xPlayer.getInventoryItem(Config.itemuse).count

    if item >= 1  then
        TriggerClientEvent('pk_bee:pic',source)        
    else        
        TriggerClientEvent("pNotify:SendNotification", source, {
			text = Config.nohave,
			type = "error",
