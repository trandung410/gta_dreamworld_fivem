			...
			if Config.EnableSocietyOwnedVehicles then
				...
				ESX.TriggerServerCallback('esx_society:getVehiclesInGarage', function(vehicles)
					...
					ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle_spawner', {
						...
						ESX.Game.SpawnVehicle(vehicleProps.model, Config.Zones.VehicleSpawnPoint.Pos, 183.5, function(vehicle)
				...
				ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'spawn_vehicle', {
					...
					if Config.MaxInService == -1 then
						ESX.Game.SpawnVehicle(data.current.value, Config.Zones.VehicleSpawnPoint.Pos, 183.5, function(vehicle)
							local playerPed = PlayerPedId()
							TaskWarpPedIntoVehicle(playerPed, vehicle, -1)
						end)
					else
	...
	elseif data.current.value == 'fix_vehicle' then
		...
		if DoesEntityExist(vehicle) then
			...
			Citizen.CreateThread(function()
				...
				TriggerServerEvent('esx_mechanicjob:NhanExp',100)
				isBusy = false
			end)
		else
			ESX.ShowNotification(_U('no_vehicle_nearby'))
	...
	elseif data.current.value == 'del_vehicle' then
		...
		if IsPedSittingInAnyVehicle(playerPed) then
			...
			if GetPedInVehicleSeat(vehicle, -1) == playerPed then
				...
				--TriggerServerEvent('esx_mechanicjob:NhanExp',25)
			else
...
function OpenMobileMechanicActionsMenu1()
...
Citizen.CreateThread(function()
	while true do
		...
		if IsControlJustReleased(0, Keys['F6']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' and ESX.PlayerData.job.grade_name == 'boss' then
			OpenMobileMechanicActionsMenu1()
		elseif IsControlJustReleased(0, Keys['F6']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			OpenMobileMechanicActionsMenu()
		end

		if IsControlJustReleased(0, Keys['F5']) and not isDead and ESX.PlayerData.job and ESX.PlayerData.job.name == 'mechanic' then
			if NPCOnJob then
...
function OpenFineMenu(player)
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'fine',
	{
		...
		align    = 'right',
		elements = {
			{label = "Khu vực trong thành phố", value = 8},
			{label = "Khu vực ngoài thành phố",   value = 9}
		}
	}, function(data, menu)
...
local blacklistVehicle = {
	...
	[GetHashKey('polchiron')] = true,
	[GetHashKey('pd_bmwr')] = true,
	[GetHashKey('pbus')] = true,
	[GetHashKey('riot')] = true,
	[GetHashKey('mc_bmwr')] = true,
	[GetHashKey('pol718')] = true,
	[GetHashKey('pd_bmw')] = true,
}
...
Citizen.CreateThread(function()
	...
	while true do 
		Wait(2000)
		if ESX.PlayerData.job.name ~= "mechanic" and ESX.PlayerData.job.name ~= "police" and ESX.PlayerData.job.name ~= "ambulance" then 
			local ped = PlayerPedId()
			local inVehicle = IsPedInAnyVehicle(ped, false)
			if inVehicle then 
				local vehicle = GetVehiclePedIsIn(ped, false)
				local vehicleModel = GetEntityModel(vehicle)
				if blacklistVehicle[vehicleModel] then 
					local driver = GetPedInVehicleSeat(vehicle, -1)
					if driver == ped then 
						ESX.ShowNotification("~r~Bạn đang điều khiển trái phép phương tiện của cơ quan nhà nước")
