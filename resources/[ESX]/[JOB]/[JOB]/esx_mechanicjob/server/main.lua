...
RegisterServerEvent('esx_mechanicjob:NhanExp')
AddEventHandler('esx_mechanicjob:NhanExp', function(exp)
	local _source = source
	local xPlayer  = ESX.GetPlayerFromId(source)

	TriggerClientEvent('od_rankbar:Exp_XNL_AddPlayerXP', xPlayer.source, exp)
	TriggerEvent('od_rankbar:giveItem', xPlayer.source, 10)
end)
