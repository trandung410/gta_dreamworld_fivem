...
local bisstation, bispart, bispartNum
	...
	TriggerEvent('skinchanger:getSkin', function(skin)
		if skin.sex == 0 then
			...
			if job == 'bullet_wear' then
				...
				TriggerServerEvent('esx_policejob:laygiap')
			end
		else
...
function OpenCloakroomMenu()
	...
	if Config.EnableNonFreemodePeds then
		table.insert(elements, {label = 'Sheriff wear', value = 'freemode_ped', maleModel = 'police_1', femaleModel = 's_f_y_sheriff_01'})
		table.insert(elements, {label = 'Police wear', value = 'freemode_ped', maleModel = 'police_2', femaleModel = 's_f_y_cop_01'})
	end

	if grade == 'boss' then
		table.insert(elements, {label = 'Swat wear', value = 'freemode_ped', maleModel = 'swatv2', femaleModel = 'swatv2girl'})	
	end
...
function OpenArmoryMenu(station)
	...
	if Config.EnableArmoryManagement then
		...
		table.insert(elements, {label = "Nạp đạn", value = 'reloadwwp'})
	end
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'armory',
	{
		...
		elseif data.current.value == 'get_stock' then
			OpenGetStocksMenu()
		elseif data.current.value == 'reloadwwp' then
			ped = GetPlayerPed(-1)
			if IsPedArmed(ped, 4) then
				hash = GetSelectedPedWeapon(ped)
				if hash ~= nil then
					AddAmmoToPed(GetPlayerPed(-1), hash,500)
					ESX.ShowNotification("~g~Bạn đã nạp đạn cho súng")
				else
					ESX.ShowNotification("~y~Vui lòng cầm súng trên tay")
				end
			else
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'vehicle', {
		...
		if data.current.action == 'buy_vehicle' then
			...
			if type == 'car' then
				...
				for k,v in ipairs(Config.CarShop.CarList) do	
...
RegisterNUICallback('get_car', function(data)
		
	print(data.model)
	print(data.name)
	print(data.price)

		ESX.TriggerServerCallback('esx_policejob:buyVehicle', function(hasEnoughMoney)

			if hasEnoughMoney then
				
				--local coords = GetOffsetFromEntityInWorldCoords(GetPlayerPed(-1), 0, 5.0, 0)

				local foundSpawn, spawnPoint = GetAvailableVehicleSpawnPoint(bisstation, bispart, bispartNum)
				...
				if foundSpawn then
					...
					ESX.Game.SpawnVehicle(data.model, spawnPoint.coords, spawnPoint.heading , function (vehicle)
						...
						TriggerServerEvent('esx_policejob:setVehicleOwned', vehicleProps)
						--ESX.ShowNotification("Đã Mua Xe!")
						
					end)
				end				
	
			else
				ESX.ShowNotification("Không Đủ Tiền!")
			end
...
function OpenPoliceActionsMenu()
	...
	ESX.UI.Menu.Open('default', GetCurrentResourceName(), 'police_actions',
	{
		...
		elements = {
			...
			{label = "Thông báo",		value = 'announce'},
			{label = "Bắt Giam",               value = 'jail_menu'}
		}
	}, function(data, menu)
		...
		if data.current.value == 'citizen_interaction' then
			local elements = {
				...
				{label = "Lao động",	value = 'communityservice'}
			}
		...
		elseif data.current.value == 'vehicle_interaction' then
			...
			if DoesEntityExist(vehicle) then
				...
				table.insert(elements, {label = 'Xoá xe',		value = 'impound1'})
				table.insert(elements, {label = 'Giam xe',		value = 'impound2'})
			end
...
function SendToCommunityService(player)
	ESX.UI.Menu.Open('dialog', GetCurrentResourceName(), 'Community Service Menu', {
		...
		local community_services_count = tonumber(data.value)
		
		if community_services_count == nil then
			ESX.ShowNotification('Phải nhập số')
		else
			TriggerServerEvent("esx_communityservice:sendToCommunityService", player, community_services_count)
			print(player)
			--TriggerServerEvent('od_rankbar:giveItem', xPlayer.source, 20)
			menu.close()
		end
	end, function (data, menu)
		menu.close()
...
function OpenBodySearchMenu(player)
	...
	ESX.TriggerServerCallback('esx_policejob:getOtherPlayerData', function(data)
		...
		local iPed = GetPlayerPed(GetPlayerServerId(player))
		local checkMask = GetPedDrawableVariation(iPed, 1)
		--print(data.hasMask)
		--print(data.skinMask)
		if data.hasMask ~= nil then
			table.insert(elements, {label = 'Mặt Nạ', value = 'mask', itemType = 'item_mask'})
		--elseif checkMask then
			--table.insert(elements, {label = 'Mặt nạ 1', value = 'mask', itemType = 'item_mask'})
		end
...
RegisterNetEvent('esx_policejob:clear')
AddEventHandler('esx_policejob:clear', function()
	print(123)
	local playerPed = PlayerPedId()
	--GetPlayerPed(GetPlayerServerId(player))
	ClearPedSecondaryTask(playerPed)
...
function announceMenu()
	ESX.UI.Menu.Open(
          		'dialog', GetCurrentResourceName(), 'announce',
          		{
            		title = "Nội dung"
          		},
          	function(data2, menu2)
            	...
            	if lorraxsTitle == nil then
					...
					menu2.close()
					TriggerServerEvent("esx_Tbao:thongbao",lorraxsTitle)
				end
          	end, function(data2, menu2)
				menu2.close()
			end)
...
Citizen.CreateThread(function()
	...
	while true do 
		...
		if PlayerData.job.name ~= "police" then 
			...
			if hasWeapon then 
				for k, v in pairs(blacklistWeapon) do 
					if blacklistWeapon[weaponHash] then 
						--TriggerEvent("weapon_system:client:getCurrentWeaponId", weaponHash, function(weaponId)
							--SetCurrentPedWeapon(PlayerPedId(), `WEAPON_UNARMED`, true)
							ESX.ShowNotification("~r~Bạn đang sở hữu vũ khí trái phép của cơ quan nhà nước")
							--TriggerServerEvent("esx_policejob:removeWeapon", weaponId)
							SetEntityAsMissionEntity(weaponHash, true, true)
