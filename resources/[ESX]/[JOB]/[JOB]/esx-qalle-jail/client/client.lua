...
function JailLogin()
	...
	ESX.ShowNotification("Lần trước bạn đang ở tù chưa hết thời hạn, nên bây giờ bạn phải ở tù tiếp tục nhé !")
...
function UnJail()
	...
	ESX.Game.Teleport(PlayerPedId(), Config.Teleports["Thoát Ra"])

	ESX.TriggerServerCallback('esx_skin:getPlayerSkin', function(skin)
		TriggerEvent('skinchanger:loadSkin', skin)
	end)

	ESX.ShowNotification("Bạn đã được tự do! Hãy làm một công dân tốt nhé!")
end

function checkInJail(idPlacex)
	idPlace = idPlacex
end
...
function InJail()
	...
	Citizen.CreateThread(function()
		...
		while jailTime > 0 do
			...
			local prison = Config.Teleports[idPlace]
			local DistanceCheck = GetDistanceBetweenCoords(PedCoords, prison["x"], prison["y"], prison["z"], true)
			if DistanceCheck >= 30.0 then
				local JailPosition = Config.Teleports[idPlace]
				SetEntityCoords(PlayerPedId(), JailPosition["x"], JailPosition["y"], JailPosition["z"] - 1)
			end
			Citizen.Wait(5000)
		end
		...
		while jailTime > 0 do
			...
			for posId, v in pairs(Packages) do
				...
				if DistanceCheck <= 10.0 then
					...
					local PackageText = "Gói Hàng"

					if not v["state"] then
						PackageText = "Đã có người bốc xếp gói hàng này !"
					end

					ESX.Game.Utils.DrawText3D(v,"<FONT FACE='Montserrat'>[E] " .. PackageText, 0.4)
					...
					if DistanceCheck <= 1.5 then
						...
						if IsControlJustPressed(0, 38) then
							...
							if v["state"] then
								...
								ESX.ShowNotification("Bạn đã thực hiện gói này !")
							end
		...
		while true do
			...
			for p, v in pairs(Config.Teleports) do
				...
				if DistanceCheck <= 7.5 then
					...
					ESX.Game.Utils.DrawText3D(v, "<FONT FACE='Montserrat'>[E] Vào Trong", 0.4)
...
function PackPackage(packageId)
	...
	while Packaging do
		...
		if PackPercent >= 100 then
			...
			ESX.Game.Utils.DrawText3D(Package, "<FONT FACE='Montserrat'>Bốc Hàng Lên.." .. math.ceil(tonumber(PackPercent)) .. "%", 0.4)
		end
...
function DeliverPackage(packageId)
	...
	while Packaging do
		...
		if not IsEntityAttachedToEntity(packageId, PlayerPedId()) then
			...
			ESX.Game.Utils.DrawText3D(DeliverPosition, "<FONT FACE='Montserrat'>[E] Đặt hàng xuống", 0.4) 
...
function OpenJailMenu()
	ESX.UI.Menu.Open(
		'default', GetCurrentResourceName(), 'jail_prison_menu',
		{
			...
			align    = 'center',
			elements = {
				{ label = "Đưa người chơi vào tù", value = "jail_closest_player" },
				{ label = "Thả người chơi tự do", value = "unjail_player" }
			}
		}, 
	function(data, menu)
			...
			ESX.UI.Menu.Open(
          	...
          	function(data2, menu2)
            	...
            	if jailTime == nil then
              		ESX.ShowNotification("Thời gian phải được tính bằng phút !")
            	else
              		menu2.close()
              		...
              		if closestPlayer == -1 or closestDistance > 3.0 then
                		ESX.ShowNotification("Không có người chơi ở gần !")
					else
						ESX.UI.Menu.Open(
						...
						function(data3, menu3)
						  	...
						  	if reason == nil then
								ESX.ShowNotification("Bạn phải đặt cái gì đó ở đây !")
						  	else
								menu3.close()
		...
		elseif action == "unjail_player" then
			...
			ESX.TriggerServerCallback("esx-qalle-jail:retrieveJailedPlayers", function(playerArray)
				...
				if #playerArray == 0 then
					ESX.ShowNotification("Nhà tù trống rỗng !") 
					return
				end

				for i = 1, #playerArray, 1 do
					table.insert(elements, {label = "Tội phạm: " .. playerArray[i].name .. " | Thời gian tù: " .. playerArray[i].jailTime .. " phút", value = playerArray[i].identifier })
				end

				ESX.UI.Menu.Open(
					'default', GetCurrentResourceName(), 'jail_unjail_menu',
					{
						title = "Thả người chơi tự do",
						align = "center",
						elements = elements
					},
				function(data2, menu2)
