...
Citizen.CreateThread(function()
	while true do
		...
		if truckdeposit then
			...
			dist = Vdist(plyCoords.x, plyCoords.y, plyCoords.z, trunk.x , trunk.y, trunk.z)
			if dist <= 2.0 then
			ESX.Game.Utils.DrawText3D(vector3(trunk.x , trunk.y ,trunk.z + 0.50), "<FONT FACE='Montserrat'>Nhấn ~g~[E]~s~ ném túi rác.", 1.0)
			end
		end
		...
		if trashcollection then
			...
			if dist <= 5.0 then
				if currentbag <= 0 then
					if iscurrentboss then
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "<FONT FACE='Montserrat'>Nhấn ~g~[E]~s~ dọn rác vụn", 1.0)		
					else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "Hốt rác xong..Chờ xe tải..", 1.0)		
					end
				else
					ESX.Game.Utils.DrawText3D(trashcollectionpos + vector3(0.0, 0.0, 1.0), "<FONT FACE='Montserrat'>Nhấn ~g~[E]~s~ để nhặt túi từ thùng rác ["..currentbag.."/"..bagsoftrash.."]", 1.0)
				end
			end
		end
